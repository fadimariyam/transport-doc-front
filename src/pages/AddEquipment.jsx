...
export default function AddEquipment() {
  ...
  const loadTypes = async () => {
    const res = await axios.get(API + "/types?category=equipment", {
  ...
  const addType = async (newType) => {
    await axios.post(
      API + "/types",
      { name: newType, category: "equipment" },
      { headers: { Authorization: "Bearer " + token } }
    );
    loadTypes();
  ...
  const addDoc = () => {
    setDocs([...docs, { name: "", expiry: "", file: null }]);
  };
        ...
        <div className="doc-list">
          {docs.map((d, i) => (
            <div key={i} className="doc-card">
              <div className="doc-head">Document {i + 1}</div>
              <div className="doc-row">
                <div className="doc-field">
                  <label>NAME</label>
                  <input
          ...
          <button className="save-doc" onClick={addDoc}>
            + Add Document
          </button>
        </div>
      </div>

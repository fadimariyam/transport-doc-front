...
export default function ViewVehicle() {
  ...
  const [statFilter, setStatFilter] = useState("all");
  const [form, setForm] = useState(null);
  const [oil, setOil] = useState({ current: 0, last: 0, interval: 0 });
  ...
  const loadVehicle = async () => {
    try {
      ...
      setOil({
        current: v.current_km || 0,
        last: v.last_oil || 0,
        interval: (v.next_oil - v.last_oil) || 0,
      });
    } catch (err) {
      console.log(err);
    }
  ...
  const loadDocs = async () => {
    try {
      const res = await axios.get(`${API}/documents`, {
        params: { item_type: "vehicle", item_id: id },
        headers: { Authorization: "Bearer " + localStorage.getItem("token") },
      });
      setDocs(res.data || []);
    } catch (err) {
  ...
  const nextOil = Number(oil.last) + Number(oil.interval);
  let oilStatus = "Normal";
  if (oil.current > nextOil) oilStatus = "Expired";
  else if (oil.current >= nextOil - 500) oilStatus = "Soon";
  ...
  const saveVehicle = async () => {
    try {
      const next = Number(oil.last) + Number(oil.interval);
      await axios.put(
        `${API}/vehicles/${id}`,
        {
          name: form.name,
      ...
      <div className="viewv-grid">
          ...
          <div className="viewv-card">
            ...
            <div className="viewv-details">
              ...
              <div>
                <span>BRAND</span>
                {edit ? (
                  <input className="viewv-input" value={form.brand || ""} onChange={(e) => setForm({ ...form, brand: e.target.value })} />
                ) : <b>{form.brand || "-"}</b>}
              </div>
              ...
              <div>
                <span>VEHICLE NAME</span>
                {edit ? (
                  <input className="viewv-input" value={form.name || ""} onChange={(e) => setForm({ ...form, name: e.target.value })} />
                ) : <b>{form.name || "-"}</b>}
              </div>
              ...
              <div>
                <span>PLATE</span>
                {edit ? (
                  <input className="viewv-input" value={form.plate || ""} onChange={(e) => setForm({ ...form, plate: e.target.value })} />
                ) : <b>{form.plate || "-"}</b>}
              </div>
              ...
              <div>
                <span>OWNER</span>
                {edit ? (
                  <input className="viewv-input" value={form.owner || ""} onChange={(e) => setForm({ ...form, owner: e.target.value })} />
                ) : <b>{form.owner || "-"}</b>}
              </div>
              ...
              <div>
                <span>CATEGORY</span>
                {edit ? (
                  <select className="viewv-input" value={form.category || "Fixed"} onChange={(e) => setForm({ ...form, category: e.target.value })}>
                    <option value="Fixed">Fixed</option>
                    <option value="Common">Common</option>
                  </select>
                ) : <b>{form.category || "-"}</b>}
              ...
              <div>
                <span>TYPE</span>
                {edit ? (
                  <input className="viewv-input" value={form.type || ""} onChange={(e) => setForm({ ...form, type: e.target.value })} />
                ) : <b>{form.type || "-"}</b>}
              </div>
              ...
              <div>
                <span>CURRENT KM</span>
                {edit ? (
                  <input className="viewv-input" value={oil.current} onChange={(e) => setOil({ ...oil, current: e.target.value })} />
                ) : <b>{oil.current}</b>}
              </div>
              ...
              <div>
                <span>LAST OIL KM</span>
                {edit ? (
                  <input className="viewv-input" value={oil.last} onChange={(e) => setOil({ ...oil, last: e.target.value })} />
                ) : <b>{oil.last}</b>}
              </div>
              ...
              <div>
                <span>INTERVAL</span>
                {edit ? (
                  <input className="viewv-input" value={oil.interval} onChange={(e) => setOil({ ...oil, interval: e.target.value })} />
                ) : <b>{oil.interval}</b>}
              </div>
              ...
              <div>
                <span>OIL STATUS</span>
                <b className={`viewv-status ${oilStatus === "Normal" ? "ok" : oilStatus === "Soon" ? "soon" : "expired"}`}>
                  {oilStatus}
                </b>
              </div>
            </div>
            ...
            <div className="viewv-doc-grid">
              {filteredDocs.map((d) => (
                <div key={d.id} className="viewv-doc">
                  ...
                  <div className="viewv-doc-actions">
                    <button onClick={() => { const u = getFileUrl(d.url); if (u) window.open(u, "_blank"); }}>
                      <FaEye />
                    </button>
        ...
        <div className="viewv-right">
          ...
          <div className="viewv-card">
            ...
            <div className="viewv-qr-text">{form.vehicle_id} — {form.name}</div>

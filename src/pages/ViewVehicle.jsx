...
export default function ViewVehicle() {
  ...
  const loadVehicle = async () => {
    try {
      ...
      setOil({
        ...
        last: v.last_oil || 0,
        interval: (v.next_oil || 0) - (v.last_oil || 0),
      });
    } catch (err) {
      console.log(err);
    }
  ...
  useEffect(() => {
    if (id) { loadVehicle(); loadDocs(); }
  }, [id]);

  /* ---- oil calc ---- */
  const nextOil = Number(oil.last) + Number(oil.interval);
  let oilStatus = "Normal";
  if (Number(oil.current) > nextOil) oilStatus = "Expired";
  else if (Number(oil.current) >= nextOil - 500) oilStatus = "Soon";
  ...
  const filteredDocs =
    statFilter === "all"
      ? allDocs
      : allDocs.filter(
          (d) => getStatus(d.expiry).toLowerCase() === statFilter
        );
          ...
          <div className="viewv-card">
            ...
            <div className="viewv-details">
              ...
              <div>
                ...
                {edit
                  ? <input type="number" value={oil.current} onChange={(e) => setOil({ ...oil, current: e.target.value })} />
                  : <b>{oil.current}</b>}
              </div>
              ...
              <div>
                ...
                {edit
                  ? <input type="number" value={oil.last} onChange={(e) => setOil({ ...oil, last: e.target.value })} />
                  : <b>{oil.last}</b>}
              </div>
              ...
              <div>
                ...
                {edit
                  ? <input type="number" value={oil.interval} onChange={(e) => setOil({ ...oil, interval: e.target.value })} />
                  : <b>{oil.interval}</b>}
              </div>
